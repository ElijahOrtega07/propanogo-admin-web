...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsubscribeUsuarios = onSnapshot(collection(firestore, "usuario"), (snapshot) => {
      const usuarios = snapshot.docs.map(doc => doc.data());
      const repartidoresActivos = usuarios.filter(u => u.rol === "repartidor" && u.activo === true);
      setRepartidores(repartidoresActivos.length);
    });
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const repartidoresCount = usuarios.filter(
          u => u.rol === "repartidor" && u.estado === "Activo"
        ).length;
